package com.app.brandmania.Model;

import com.app.brandmania.databinding.LayoutForLoadEightBinding;
import com.app.brandmania.databinding.LayoutForLoadFiveBinding;
import com.app.brandmania.databinding.LayoutForLoadFourBinding;
import com.app.brandmania.databinding.LayoutForLoadNineBinding;
import com.app.brandmania.databinding.LayoutForLoadOneBinding;
import com.app.brandmania.databinding.LayoutForLoadSevenBinding;
import com.app.brandmania.databinding.LayoutForLoadSixBinding;
import com.app.brandmania.databinding.LayoutForLoadTenBinding;
import com.app.brandmania.databinding.LayoutForLoadThreeBinding;
import com.app.brandmania.databinding.LayoutForLoadTwoBinding;

public class LayoutModelClass{
    private LayoutForLoadOneBinding oneBinding;
    private LayoutForLoadTwoBinding twoBinding;
    private LayoutForLoadThreeBinding threeBinding;
    private LayoutForLoadFourBinding fourBinding;
    private LayoutForLoadFiveBinding fiveBinding;
    private LayoutForLoadSixBinding sixBinding;
    private LayoutForLoadSevenBinding sevenBinding;
    private LayoutForLoadEightBinding eightBinding;
    private LayoutForLoadNineBinding nineBinding;
    private LayoutForLoadTenBinding tenBinding;

    public LayoutForLoadOneBinding getOneBinding() {
        return oneBinding;
    }

    public LayoutModelClass setOneBinding(LayoutForLoadOneBinding oneBinding) {
        this.oneBinding = oneBinding;
        return this;
    }

    public LayoutForLoadTwoBinding getTwoBinding() {
        return twoBinding;
    }

    public LayoutModelClass setTwoBinding(LayoutForLoadTwoBinding twoBinding) {
        this.twoBinding = twoBinding;
        return this;
    }

    public LayoutForLoadThreeBinding getThreeBinding() {
        return threeBinding;
    }

    public LayoutModelClass setThreeBinding(LayoutForLoadThreeBinding threeBinding) {
        this.threeBinding = threeBinding;
        return this;
    }

    public LayoutForLoadFourBinding getFourBinding() {
        return fourBinding;
    }

    public LayoutModelClass setFourBinding(LayoutForLoadFourBinding fourBinding) {
        this.fourBinding = fourBinding;
        return this;
    }

    public LayoutForLoadFiveBinding getFiveBinding() {
        return fiveBinding;
    }

    public LayoutModelClass setFiveBinding(LayoutForLoadFiveBinding fiveBinding) {
        this.fiveBinding = fiveBinding;
        return this;
    }

    public LayoutForLoadSixBinding getSixBinding() {
        return sixBinding;
    }

    public LayoutModelClass setSixBinding(LayoutForLoadSixBinding sixBinding) {
        this.sixBinding = sixBinding;
        return this;
    }

    public LayoutForLoadSevenBinding getSevenBinding() {
        return sevenBinding;
    }

    public LayoutModelClass setSevenBinding(LayoutForLoadSevenBinding sevenBinding) {
        this.sevenBinding = sevenBinding;
        return this;
    }

    public LayoutForLoadEightBinding getEightBinding() {
        return eightBinding;
    }

    public LayoutModelClass setEightBinding(LayoutForLoadEightBinding eightBinding) {
        this.eightBinding = eightBinding;
        return this;
    }

    public LayoutForLoadNineBinding getNineBinding() {
        return nineBinding;
    }

    public LayoutModelClass setNineBinding(LayoutForLoadNineBinding nineBinding) {
        this.nineBinding = nineBinding;
        return this;
    }

    public LayoutForLoadTenBinding getTenBinding() {
        return tenBinding;
    }

    public LayoutModelClass setTenBinding(LayoutForLoadTenBinding tenBinding) {
        this.tenBinding = tenBinding;
        return this;
    }
}